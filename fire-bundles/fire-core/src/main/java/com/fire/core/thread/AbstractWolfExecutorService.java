 ...
 * @author Doug Lea
 */
public abstract class AbstractWolfExecutorService implements ExecutorService {
	 ...
	 *         runnable and which, as a {@code Future}, will yield the given
	 *         value as its result and provide for cancellation of the
	 *         underlying task
	 * @since 1.6
	 */
	protected <T> RunnableFuture<T> newTaskFor(Runnable runnable, T value) {
		return new WolfFutureTask<T>(runnable, value,
				((WolfRunnable) runnable).getContext());
	}
	...
	protected <T> RunnableFuture<T> newTaskFor(Callable<T> callable,
			WolfWebContext context) {
		return new WolfFutureTask<T>(callable, context);
	}
	...
	private <T> T doInvokeAny(Collection<? extends Callable<T>> tasks,
		...
		ExecutorCompletionService<T> ecs = new ExecutorCompletionService<T>(
				this);
	...
	public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks)
		...
		try {
			for (Callable<T> t : tasks) {
				RunnableFuture<T> f = newTaskFor(t, null);
				futures.add(f);
				execute(f);
			}
			for (int i = 0, size = futures.size(); i < size; i++) {
