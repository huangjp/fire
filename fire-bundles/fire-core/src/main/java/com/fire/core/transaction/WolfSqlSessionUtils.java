 ...
 * @version $Id$
 */
public final class WolfSqlSessionUtils {

	private static final Log logger = LogFactory
			.getLog(WolfSqlSessionUtils.class);

	/**
	 * This class can't be instantiated, exposes static utility methods only.
	 */
	private WolfSqlSessionUtils() {
		// do nothing
	}
	...
	/**
	 * Gets an SqlSession from Spring Transaction Manager or creates a new one
	 * if needed. Tries to get a SqlSession out of current transaction. If there
	 * is not any, it creates a new one. Then, it synchronizes the SqlSession
	 * with the transaction if Spring TX is active and
	 * <code>SpringManagedTransactionFactory</code> is configured as a
	 * transaction manager.
	 *
	 * @param sessionFactory
	 *            a MyBatis {@code SqlSessionFactory} to create new sessions
	...
	public static SqlSession getSqlSession(SqlSessionFactory sessionFactory,
		...
		WolfSqlSessionHolder holder = (WolfSqlSessionHolder) TransactionSynchronizationManager
		...
		if (holder != null && holder.isSynchronizedWithTransaction()) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Fetched SqlSession [" + holder.getSqlSession()
		...
		// Register session holder if synchronization is active (i.e. a Spring
		// TX is active)
		//
		// Note: The DataSource used by the Environment should be synchronized
		// with the
		// transaction either through DataSourceTxMgr or another tx
		// synchronization.
		...
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			if (environment.getTransactionFactory() instanceof SpringManagedTransactionFactory) {
				...
				holder = new WolfSqlSessionHolder(session, executorType,
						exceptionTranslator);
				TransactionSynchronizationManager.bindResource(sessionFactory,
						holder);
				TransactionSynchronizationManager
						.registerSynchronization(new WolfSqlSessionSynchronization(
	 ...
	 */
	private static final class WolfSqlSessionSynchronization extends
			TransactionSynchronizationAdapter {

		private final WolfSqlSessionHolder holder;

		private final SqlSessionFactory sessionFactory;

		private boolean holderActive = true;

		public WolfSqlSessionSynchronization(WolfSqlSessionHolder holder,
				SqlSessionFactory sessionFactory) {
		...
		public void beforeCommit(boolean readOnly) {
			...
			// connection
			// TODO This updates 2nd level caches but the tx may be rolledback
			// later on!
