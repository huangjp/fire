...
public class WolfFutureTask<V> extends WolfRunnable implements
		RunnableFuture<V> {
	/*
	...
	private static final long serialVersionUID = -6972661243008781933L;
	/**
	 ...
	 */
	public WolfFutureTask(Callable<V> callable, WolfWebContext context) {
		this(context);
		if (callable == null) {
	 ...
	 */
	public WolfFutureTask(Runnable runnable, V result, WolfWebContext context) {
		this(context);
	...
	public WolfFutureTask(WolfWebContext context) {
		super(context);
	}
	...
	public boolean cancel(boolean mayInterruptIfRunning) {
		if (!(state == NEW && UNSAFE.compareAndSwapInt(this, STATE_OFFSET, NEW,
		...
		try { // in case call to interrupt throws exception
			if (mayInterruptIfRunning) {
				try {
					...
					UNSAFE.putOrderedInt(this, STATE_OFFSET, INTERRUPTED);
				}
			}
		} finally {
			finishCompletion();
	...
	protected void set(V v) {
		if (UNSAFE.compareAndSwapInt(this, STATE_OFFSET, NEW, COMPLETING)) {
			outcome = v;
			// final state
			UNSAFE.putOrderedInt(this, STATE_OFFSET, NORMAL); 
			finishCompletion();
		}
	}
	...
	protected void setException(Throwable t) {
		if (UNSAFE.compareAndSwapInt(this, STATE_OFFSET, NEW, COMPLETING)) {
			outcome = t;
			// final state
			UNSAFE.putOrderedInt(this, STATE_OFFSET, EXCEPTIONAL); 
			finishCompletion();
		}
	}
	...
	public void run() {
		if (state != NEW
				|| !UNSAFE.compareAndSwapObject(this, RUNNER_OFFSET, null,
	...
	private int awaitDone(boolean timed, long nanos)
		...
		for (;;) {
			...
			if (s > COMPLETING) {
				...
				queued = UNSAFE.compareAndSwapObject(this, WAITERS_OFFSET,
						q.next = waiters, q);
			} else if (timed) {
		...
		if (node != null) {
			...
			retry: for (;;) { // restart on removeWaiter race
				for (WaitNode pred = null, q = waiters, s; q != null; q = s) {
					...
					} else if (!UNSAFE.compareAndSwapObject(this,
							WAITERS_OFFSET, q, s)) {
						continue retry;
					}
				}
	...
	private static final long RUNNER_OFFSET;
	private static final long WAITERS_OFFSET;
	static {
		try {
			Field f = Unsafe.class.getDeclaredField("theUnsafe");   
			f.setAccessible(true);  
			UNSAFE = (Unsafe) f.get(null); 
			Class<?> k = WolfFutureTask.class;
			STATE_OFFSET = UNSAFE.objectFieldOffset(k.getDeclaredField("state"));
			RUNNER_OFFSET = UNSAFE.objectFieldOffset(k
					.getDeclaredField("runner"));
			WAITERS_OFFSET = UNSAFE.objectFieldOffset(k
					.getDeclaredField("waiters"));
		} catch (Exception e) {
			throw new Error(e);
		}
	}
