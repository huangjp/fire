package com.fire.third.party.bdb.storedMap;
...
import com.fire.third.party.bdb.config.PropertiesUtils;
...
/**
 * @author hjp
 * @ClassName: BDBStoredMapFactoryImpl
 * @date 2018年10月12日 下午3:58:00
 */
@Component(immediate = true, name = "com.fire.third.party.api.StoredMapFactory")
public class BDBStoredMapFactoryImpl
        implements StoredMapFactory<Serializable, VersionObject<Serializable>> {
    private static final Logger logger = LoggerFactory
            .getLogger(BDBStoredMapFactoryImpl.class);
    private final ConcurrentHashMap<String, QueueEnvironment> envMap = new ConcurrentHashMap<String, QueueEnvironment>();

    private final ConcurrentHashMap<String, StoredMap<Serializable, VersionObject<Serializable>>> storedMaps = new ConcurrentHashMap<String, StoredMap<Serializable, VersionObject<Serializable>>>();
     ...
     */
    @Override
    public synchronized ConcurrentMap<Serializable, VersionObject<Serializable>> buildMap(
        ...
        FstSerialBinding<VersionObject<Serializable>> messageValueBinding = new FstSerialBinding<VersionObject<Serializable>>();
        Database db = env.buildDatabase(name);

        String keyName = new StringBuilder().append(storedpath).append(name)
                .toString();
        StoredMap<Serializable, VersionObject<Serializable>> map = storedMaps.get(keyName);
        if (map == null) {
            StoredMap<Serializable, VersionObject<Serializable>> tmpMap = new StoredMap<Serializable, VersionObject<Serializable>>(
                    db, messageKeyBinding, messageValueBinding, true);
            StoredMap<Serializable, VersionObject<Serializable>> old = storedMaps
                    .putIfAbsent(keyName, tmpMap);
            return old == null ? tmpMap : old;
        }
        return map;
    }

    private StoredSortedMap<Long, Serializable> buildStoredSortedMap(
            String storedpath, String name) {
        QueueEnvironment env = buildBDB(storedpath);
        // SerialBinding<Long> messageKeyBinding = new
        // SerialBinding<Long>(env.getStoredClassCatalog(), Long.class);
        // SerialBinding<Message> messageValueBinding = new
        // SerialBinding<Message>(env.getStoredClassCatalog(), Message.class);
        ...
        StoredSortedMap<Long, Serializable> soredMap = sortedstoredMap
                .get(keyName);

        if (soredMap == null) {
            soredMap = new StoredSortedMap<Long, Serializable>(db,
                    (EntryBinding<Long>) messageKeyBinding,
                    (EntryBinding<Serializable>) messageValueBinding, true);

            StoredSortedMap<Long, Serializable> old = sortedstoredMap
                    .putIfAbsent(keyName, soredMap);
    ...
    private class QueueEnvironment {
        ...
        public QueueEnvironment buildEnvironment(String pathHome) {
            ...
            EnvironmentConfig environmentConfig = new EnvironmentConfig(
                    PropertiesUtils.getJeProperties());
        ...
        private void cleanLogSchedule() {
            threadService.getBusiwork().scheduleWithFixedDelay(new Runnable() {
                @Override
                public void run() {
                    clearLog();
                }
