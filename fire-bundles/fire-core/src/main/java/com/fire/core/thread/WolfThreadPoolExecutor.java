 ...
 *
 * <dd>New threads are created using a {@link WolfThreadFactory}. If not
 * otherwise specified, a {@link Executors#defaultThreadFactory} is used, that
 * creates threads to all be in the same {@link ThreadGroup} and with the same
 ...
 * {@link #setKeepAliveTime(long, TimeUnit)}. Using a value of
 * {@code Long.MAX_VALUE} {@link TimeUnit#NANOSECONDS} effectively disables idle
 * threads from ever terminating prior to shut down. By default, the keep-alive
 * policy applies only when there are more than corePoolSize threads. But method
 ...
 * and is saturated. In either case, the {@code execute} method invokes the
 * {@link WolfRejectedExecutionHandler#rejectedExecution(Runnable, WolfThreadPoolExecutor)}
 * method of its {@link WolfRejectedExecutionHandler}. Four predefined handler
 * policies are provided:
 *
 * <ol>
 *
 * <li>In the default {@link WolfThreadPoolExecutor.AbortPolicy}, the handler
 * throws a runtime {@link RejectedExecutionException} upon rejection.</li>
 *
 * <li>In {@link WolfThreadPoolExecutor.CallerRunsPolicy}, the thread that
 * invokes {@code execute} itself runs the task. This provides a simple feedback
 * control mechanism that will slow down the rate that new tasks are
 * submitted.</li>
 *
 * <li>In {@link WolfThreadPoolExecutor.DiscardPolicy}, a task that cannot be
 * executed is simply dropped.</li>
 *
 * <li>In {@link WolfThreadPoolExecutor.DiscardOldestPolicy}, if the executor is
 ...
 * {@link WolfRejectedExecutionHandler} classes. Doing so requires some care
 ...
 * @author Doug Lea
 */
public class WolfThreadPoolExecutor extends AbstractWolfExecutorService {
...
//	private final Logger log = LoggerFactory.getLogger(getClass());
	 ...
	 * OutOfMemoryError.
	 */
	private volatile WolfThreadFactory threadFactory;

	/**
	 * Handler called when saturated or shutdown in execute.
	 */
	private volatile WolfRejectedExecutionHandler handler;
	 ...
	 */
	private static final WolfRejectedExecutionHandler DEFAULT_HANDLER = new AbortPolicy();
	 ...
	 * runWorker).
	 */
	private final class Worker extends WolfRunnable {
		/**
		...
		final WolfThread thread;
		/** Initial task to run. Possibly null. */
		Runnable firstTask;
		/** Per-thread task counter */
		volatile LongAdder completedTasks = new LongAdder();
		...
		Worker(Runnable firstTask) {
			...
			WolfWebContext context = null;
			if (firstTask != null) {
				if(WolfRunnable.class.equals(this.firstTask.getClass()) 
						|| this.firstTask.getClass().equals(WolfFutureTask.class)) {
					context = ((WolfRunnable)this.firstTask).getContext();
				}
			}
				
			this.thread = (WolfThread) getThreadFactory().newThread(this,
					context);
		}
		...
		protected boolean tryAcquire(int unused) {
			if (compareAndSetState(0, 1)) {
				setExclusiveOwnerThread(WolfThread.currentThread());
				return true;
			}
			return false;
		}
	...
	private void processWorkerExit(Worker w, boolean completedAbruptly) {
		...
		try {
			...
			completedTaskCount += w.completedTasks.longValue();
			workers.remove(w);
		} finally {
			mainLock.unlock();
		}
...
//		this.log.warn("content:{}, thread name:{}", w.context,
//				WolfThread.currentThread().getName());
	}
	 ...
	 * and after the timed wait, and if the queue is non-empty, this worker is
	 * not the last thread in the pool.
	 *
		...
		for (;;) {
			...
			try {
				Runnable r = timed
						...
						: workQueue.take();
				if (r != null) {
					if(r.getClass().equals(WolfRunnable.class) || WolfFutureTask.class.equals(r.getClass())) {
						WolfWebContext context = ((WolfRunnable) r).getContext();
						WolfThread thread = (WolfThread) Thread.currentThread();
	...
	final void runWorker(Worker w) {
		WolfThread wt = (WolfThread) Thread.currentThread();
	 ...
	 */
	public WolfThreadPoolExecutor(int corePoolSize, int maximumPoolSize,
			long keepAliveTime, TimeUnit unit,
			BlockingQueue<Runnable> workQueue) {
		this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
				WolfExecutors.defaultThreadFactory(), DEFAULT_HANDLER);
	}
	...
	public WolfThreadPoolExecutor(int corePoolSize, int maximumPoolSize,
			long keepAliveTime, TimeUnit unit,
			BlockingQueue<Runnable> workQueue,
			WolfThreadFactory threadFactory) {
		this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
	...
	public WolfThreadPoolExecutor(int corePoolSize, int maximumPoolSize,
			long keepAliveTime, TimeUnit unit,
			BlockingQueue<Runnable> workQueue,
			WolfRejectedExecutionHandler handler) {
		this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
				WolfExecutors.defaultThreadFactory(), handler);
	}
	 ...
	 */
	public void setThreadFactory(WolfThreadFactory threadFactory) {
	 ...
	 * @see #setThreadFactory(WolfThreadFactory)
	 */
	public WolfThreadFactory getThreadFactory() {
	 ...
	 * @see #setRejectedExecutionHandler(WolfRejectedExecutionHandler)
	 */
	public WolfRejectedExecutionHandler getRejectedExecutionHandler() {
		return handler;
	}
	...
	public String toString() {
		...
		try {
			...
			for (Worker w : workers) {
				...
				ncompleted += w.completedTasks.longValue();
				if (w.isLocked()) {
					++nactive;
				}
			}
		...
		String rs = (runStateLessThan(c, SHUTDOWN) ? "Running"
				: (runStateAtLeast(c, TERMINATED) ? "Terminated"
						: "Shutting down"));
		return super.toString() + "[" + rs + ", pool size = " + nworkers
				+ ", active threads = " + nactive + ", queued tasks = "
	...
	public static class CallerRunsPolicy
			implements WolfRejectedExecutionHandler {
		/**
	 ...
	 */
	public static class AbortPolicy implements WolfRejectedExecutionHandler {
		/**
	 ...
	 */
	public static class DiscardPolicy implements WolfRejectedExecutionHandler {
		/**
